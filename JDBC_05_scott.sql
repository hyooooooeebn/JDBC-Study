...
INSERT INTO TBL_BUSEO(BUSEO_ID, BUSEO_NAME) VALUES (BUSEOSEQ.NEXTVAL,'총모부');
...
--==>> 커밋 완료.
-----------------------------------------------------------------------------------------------------------
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_ID, TEL, 
      (SELECT FROM WHERE BUSEO_ID )BUSEO_ID, JIKWI_ID, BASICPAY, SUDANG, (BASICPAY*12)+SUDANG AS PAY
FROM TBL_EMP 
ORDER BY EMP_ID
;

SELECT E.EMP_ID AS EID, E.EMP_NAME AS NAME, E.SSN AS SSN, E.IBSADATE AS IBSADATE, C.CITY_NAME AS CITY, E.TEL AS TEL, B.BUSEO_NAME AS BUSEO 
, J.JIKWI_NAME AS JIKWI, E.BASICPAY AS BASICPAY, E.SUDANG AS SUDANG, (E.BASICPAY*12)+E.SUDANG AS PAY
...
SELECT E.EMP_ID AS EID, E.EMP_NAME AS NAME, E.SSN AS SSN, TO_CHAR(E.IBSADATE,'YYYY-MM-DD') AS IBSADATE, C.CITY_NAME AS CITY, E.TEL AS TEL, B.BUSEO_NAME AS BUSEO 
, J.JIKWI_NAME AS JIKWI, E.BASICPAY AS BASICPAY, E.SUDANG AS SUDANG, (E.BASICPAY*12)+E.SUDANG AS PAY
FROM TBL_EMP E JOIN TBL_BUSEO B
ON E.BUSEO_ID = B.BUSEO_ID
JOIN TBL_JIKWI J
...
JOIN TBL_CITY C
 ON C.CITY_ID = E.CITY_ID
WHERE J.JIKWI_NAME =  '대리';

UPDATE TBL_EMP SET EMP_NAME = '%S', SSN = '%S', IBSADATE ='%S', CITY_ID = %S, TEL = '%S', BUSEO_ID = %S , JIKWI_ID =%S , BASCIPAY = %S, SUNDANG = %S WHERE EMP_ID = %S
;
...
UPDATE TBL_EMP
SET EMP_NAME = '박효빈', SSN = 
WHERE EMP_ID = 1061;

UPDATE TBL_EMP
SET EMP_NAME = '박효빈', SSN = '980611-2222222', IBSADATE ='2021-09-05', CITY_ID = 1, TEL = '010-5541-2644'
 , BUSEO_ID = 1 , JIKWI_ID = 1 , BASICPAY = 2500000, SUDANG = 500000 
WHERE EMP_ID = 1061;

SELECT CITY_NAME FROM TBL_CITY ORDER BY CITY_ID;
...
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD') AS IBSADATE, CITY_NAME, NVL(TEL, '전화번호 없음') AS TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW ORDER BY EMP_ID
;
--이름
...
;
--급여 내림차순
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD') AS IBSADATE,  CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW ORDER BY PAY
;
...
;
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD') AS IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW WHERE EMP_NAME = '홍길동'
;
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD') AS IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW WHERE BUSEO_NAME = '개발부'
;
SELECT EMP_ID, EMP_NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD') AS IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, PAY FROM EMPVIEW WHERE JIKWI_NAME = '과장'
;

-- 10. 직원 수정
UPDATE TBL_EMP SET EMP_NAME = '이찬호', SSN = '980611-1222222', IBSADATE =TO_DATE('2021-09-05','YYYY-MM-DD'), CITY_ID = (SELECT CITY_ID FROM TBL_CITY WHERE CITY_NAME = '제주'), TEL = '010-5541-2644'  , BUSEO_ID =(SELECT BUSEO_ID FROM TBL_BUSEO WHERE BUSEO_NAME = '개발부') , JIKWI_ID =(SELECT JIKWI_ID FROM TBL_JIKWI WHERE JIKWI_NAME = '과장') , BASICPAY = 2500000, SUDANG = 500000 WHERE EMP_ID = 1064
;
